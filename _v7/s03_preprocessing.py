...
class StockPreprocessor:
    def __init__(self, stock_code=None, target_epsilon=0.005):
        """
        Args:
            target_epsilon: 보합(1)을 결정하는 수익률 임계값 (예: 0.01은 +-1%)
        """
    ...
    def _create_target_labels(self, df):
        """3진 분류 타겟 생성 (내일 종가 수익률 기준)"""
        print(f"🎯 타겟 레이블 생성 중 (Epsilon: {self.target_epsilon})...")
        
        # 내일 수익률 계산
        df['next_rtn'] = df['종가'].pct_change().shift(-1)
        
        # 레이블링 로직
        # 2: 상승 (> epsilon), 1: 보합, 0: 하락 (< -epsilon)
        conditions = [
            (df['next_rtn'] > self.target_epsilon),
            (df['next_rtn'] < -self.target_epsilon)
        ]
        choices = [2, 0]
